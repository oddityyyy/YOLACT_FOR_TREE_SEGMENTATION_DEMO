...
Yolact::Yolact(AAssetManager *mgr, bool useGPU) {
    ...
    YolactNet->load_param(mgr, "yolact-sim-opt.param");
    YolactNet->load_model(mgr, "yolact-sim-opt.bin");
...
std::vector<Object> Yolact::detect_yolact(JNIEnv *env, jobject image) {
    ...
    ex.input("input.1", in);
    ex.extract("595", maskmaps);   // 138x138 x 32
    ex.extract("792", location);   // 4 x 19248
    ex.extract("794", mask);       // maskdim 32 x 19248
    ex.extract("796", confidence); // 81 x 19248
